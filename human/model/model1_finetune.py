...
class Discriminator(nn.Module):
    ...
    def forward(self, x, y):
        ...
        scores = torch.sum(xx * self.w, dim=1, keepdim=False)
        scores = scores + self.b

        if score_normalize:
            scores = torch.tanh(scores)

        return {'d_scores': scores, 'd_features': features}
...
class Loss_EG(nn.Module):
    ...
    def forward(self, x, x_hat, d_out, d_out_hat):
        ...
        return {'1_content_loss': content_loss, '2_vgg19_loss': vgg19_loss, '3_adv_loss': adv_loss,
                '4_fm_loss': fm_loss, 'eg_loss': loss_all}


def initialize(_, __, ___, L_EG, L_DSC):
